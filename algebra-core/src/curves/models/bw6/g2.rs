    ...
    fn from(g2: G2Affine<P>) -> Self {
        ...
        for (idx, value) in P::ATE_LOOP_COUNT.iter().rev().enumerate() {
            ...
            for bit in v.iter().rev() {
                let (r2, coeff) = BW6::<P>::doubling_step_for_flipped_miller_loop(&r);
                g2p.double_coefficients.push(coeff);
                r = r2;

                if *bit {
                    let (r2, coeff) =
                        BW6::<P>::mixed_addition_step_for_flipped_miller_loop(&g2.x, &g2.y, &r);
        ...
        if P::ATE_IS_LOOP_COUNT_NEG {
            ...
            let add_result = BW6::<P>::mixed_addition_step_for_flipped_miller_loop(
                &minus_r_affine_x,
                &minus_r_affine_y,
                &r,
            );
...
pub(super) struct G2ProjectiveExtended<P: BW6Parameters> {
...
pub struct AteDoubleCoefficients<P: BW6Parameters> {
...
pub struct AteAdditionCoefficients<P: BW6Parameters> {
